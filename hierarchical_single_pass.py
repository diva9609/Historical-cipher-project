...
def context_match_score(suspect_context_clusters, cluster_context_clusters):
    ...
    matches = sum(1 for cluster_id in cluster_context_clusters
                  if cluster_id in suspect_context_set)
...
def get_accepted_candidates(features,
                            effective_labels,
                            n_clusters,
                            cluster_scores,
                            min_cluster_size,
...
def load_trigram_context(trigram_h5_path, enable_context_refinement):
    ...
    print(f"Loading trigram context from {trigram_h5_path}...")
...
def build_filename_to_cluster(filenames, effective_labels):
    ...
    return {os.path.splitext(filenames[index])[0]: int(effective_labels[index]) for index in range(len(filenames))}
...
def run_context_refinement(features,
                           filenames,
                           effective_labels,
                           n_clusters,
                           cluster_scores,
    ...
    if not (enable_context_refinement and trigram_context):
        ...
        return reassigned_by_context

    print("\nRunning context-based reassignment refinement...")
    ...
    for label in range(n_clusters):
        ...
        for suspect_local_index in suspect_order:
            ...
            if best_label is not None and best_matches > 0:
                old_label = current_label

                reassigned_by_context[suspect_global_index] = {"from": old_label, "to": best_label}
                ...
                print(f"Moved suspect {filenames[suspect_global_index]} "
    ...
    print(f"Context refinement reassigned {len(reassigned_by_context)} symbols")
...
def save_clusters(results_folder, filenames, images, effective_labels, n_clusters, cluster_scores, var_threshold, final_accepted_labels, cluster_representatives):
    ...
    for label in range(n_clusters):
        ...
        for index in indices:
            ...
            if (label in final_accepted_labels and representative_index is not None and index == representative_index):
...
def save_summary(results_folder,
    ...
    with open(summary_path, "w", encoding="utf-8") as file:
        ...
        file.write(f"Reassigned by context: {len(reassigned_by_context)}\n")
...
def main():
    ...
    if valid_scores:
        var_threshold = np.percentile(list(valid_scores.values()), 90)
    else:
        var_threshold = 0

    reassigned_by_context = run_context_refinement(features,
                                                   filenames,
                                                   effective_labels,
                                                   n_clusters,
                                                   cluster_scores,
    ...
    cluster_scores, _ = compute_cluster_scores(features,
                                               effective_labels,
                                               n_clusters,
                                               MIN_CLUSTER_SIZE)
    ...
    cluster_representatives = get_cluster_representatives(features,
                                                          effective_labels,
                                                          n_clusters)
